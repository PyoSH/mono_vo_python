...
def main():
    ...
    for index in range(dataset.image_count):
        ...
        # main process
        keypoint = cv2.goodFeaturesToTrack(image, 0, 0.1, 10)
        ...
        dst1 = cv2.cvtColor(prev_image.copy(), cv2.COLOR_GRAY2BGR)
        if points is not None:
            for i in range(prev_keypoint.shape[0]):
                pt = (int(prev_keypoint[i,0,0]), int(prev_keypoint[i,0,1]))
                cv2.circle(dst1, pt, 2, (0,0,255), 2)
